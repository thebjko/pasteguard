...
secrets_detection:
  entities:
    ...
    - KOREAN_DRIVERS_LICENSE   # 운전면허번호 (11-00-123456-78)
    - KOREAN_BANK_ACCOUNT      # 계좌번호 (110-123-456789)
```

### Field Name-Based Masking (opt-in)

Masks values of sensitive field names in YAML, TOML, and JSON structures.

```yaml
secrets_detection:
  entities:
    - FIELD_SENSITIVE_VALUE  # Masks values of fields like ssn, salary, password, etc.
  sensitive_fields:          # Optional: add custom field names
    - employee_id
    - 사번
```

See [Korean PII & Field Masking](/configuration/korean-pii-and-field-masking) for full details.
